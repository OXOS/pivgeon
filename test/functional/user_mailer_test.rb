  ...
  context "Mailer" do        
    ...
    context "when user is not created" do
      ...
      should "send notification which contains error messages" do
        ...
        assert_equal "wojciech@example.com", email.to.first
        assert_equal "Re: 12345678", email.subject
        assert_match /You or somebody else tried to create new user account in Pivgeondev application using this email: #{user.email}. Unfortunately the account couldn't be created due to following errors:/, email.encoded
      ...
      should "send notification which contains custom error message" do    
        params = valid_params('wojciech@example.com','pivgeon@pivgeon.com','','12345678')
        ...
        assert_equal "wojciech@example.com", email.to.first
        assert_equal "Re: 12345678", email.subject
        assert_match /You or somebody else tried to create new user account in Pivgeondev application using this email: wojciech@example.com. Unfortunately the account couldn't be created due to following errors:/, email.encoded
