...
class UserMailerTest < ActionMailer::TestCase
  ...
  context "Mailer" do        
    ...
    should "send notification when user account is created" do
      user = users(:wojciech)
      email =  UserMailer.created_for_creator(user).deliver!
    ...
    context "when user is not created" do
      ...
      should "send notification which contains error messages" do
        ...
        email =  UserMailer.not_created_for_creator(user).deliver!
        assert !ActionMailer::Base.deliveries.empty?
        assert_equal "wojciech@example.com", email.to.first
      ...
      should "send notification which contains custom error message" do        
        ...
        email =  UserMailer.not_created_for_creator(message,"This is custom error message").deliver!
        assert !ActionMailer::Base.deliveries.empty?
        assert_equal "wojciech@example.com", email.to.first
        assert_equal "GeePivoMailin: create new account error", email.subject
