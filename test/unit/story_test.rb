...
class StoryTest < ActiveSupport::TestCase
  ...
  context "A model" do
    ...
    should "create new story and post to pivotaltracker" do
      mock_request
      assert Story.create(@params)
    end
    
    should "parse incoming message" do
      puts "###################### #{@params.inspect}"
      assert_equal story_attrs, Story.parse_message_and_set_token(@params)
    end
    
    should "set token in headers" do
      message = incoming_params("wojciech@example.com","daniel@example.com")
      Story.parse_message_and_set_token(message)
      assert_equal '12345678', Story.headers['X-TrackerToken']
      
      message = incoming_params("faked_from@example.com","faked_to@example.com")
      Story.parse_message_and_set_token(message)
      assert Story.headers['X-TrackerToken'].blank?
      
      message = incoming_params("daniel@example.com","wojciech@example.com")
      Story.parse_message_and_set_token(message)
